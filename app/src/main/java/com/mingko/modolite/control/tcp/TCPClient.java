...
public class TCPClient implements Runnable {
    ...
    public static final int MESSAGE_HTML = 11;
    public static final int MESSAGE_AUDIO = 12;
    public static final int MESSAGE_IMAGE = 13;
    ...
    public void run() {
        try {
            EventBus.getDefault().post(MsgBean.createModoTextAnswer("正在与服务器 " + ip + ": " + port + "连接......"));
        ...
        new Thread() {
            ...
            public void run() {
                ...
                while (life){
                    try {
                        while (life && (content = br.readLine()) != null && content.trim().length() > 0) {
                            // 每当读到来自服务器的数据之后，发送消息通知程序
                            // 界面显示该数据
                            if(Math.random() < 0.2){
                                content = "<div class=\"titlebold\"><br/>春节倒计时</div><div class=\"time\"><span id=\"t_d\">00天</span><span id=\"t_h\">00时</span><span id=\"t_m\">00分</span><span id=\"t_s\">00秒</span></div>";
                            }
                            Message msg = new Message();
                            msg.what = MESSAGE_RECEIVED;
                            msg.obj = content;
                            if(content.trim().toLowerCase().startsWith("<div")){
                                msg.arg1 = MESSAGE_HTML;
                            }else {
                                msg.arg1 = MESSAGE_TEXT;
                            }
